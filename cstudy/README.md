...
```
#include <stdio.h>#define SIZE 5int main() {    int source[SIZE] = {1, 2, 3, 4, 5};    int destination[SIZE];        int *src_ptr = source;    int *dest_ptr = destination;        for (int i = 0; i < SIZE; i++) {        *dest_ptr++ = *src_ptr++;    }        printf("Copied array: ");    for (int i = 0; i < SIZE; i++) {        printf("%d ", destination[i]);    }        return 0;}
```
